...
```{r}
fat <- read.csv(file = "E:/WISC/Fall 2022/STAT 628/STAT628-Module2-Group9/Data/df1_v2.csv",header=T)
fat=fat[,-1]
head(fat)
```
```{r}
...
mlr <- lm( X5 ~ X2 + X3 +X4 + X6 + X7 + X8 + X9 + X10 + X11 + X12 + X13 + X14 + X15, fat)
summary(mlr)
summary(aov(mlr))
...
```{r}
# 全模型的线性回归(还没有变量选择的不显著的变量也在模型里面)
mlr2 <- lm(Y ~ X2 +  X6 + X8 + X10 + X15, fat)
summary(mlr2)
summary(aov(mlr2))
...
```{r}
#逐步回归（对全模型的向前和向后回归）
mlr.step = lm(Y ~ X2 +X5+  X6 + X8 + X10 + X15, fat)
summary(mlr.step)$adj.r.squared
step(mlr.step, direction = "backward")
step(mlr.step, direction = "both")
```
...
model3<-lm(Y ~ X2 + X8 + X15 , fat_first)
model4<-lm(Y ~ X2 + X5 + X6 + X8 + X15, fat)
model5<-lm(Y ~ X2 + X5 +X8 + X15, fat)
...
par(mfrow = c(1, 1))
# Independence
plot(resid(mlr3))+abline(h=0)
# Normality
qqnorm(rstandard(mlr3))
# Linearity
plot(fat$Y, rstandard(mlr3)) +
abline(h = 0)
